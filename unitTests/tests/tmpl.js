...
test("Core :: Repeat statement random in range", function () {
	...
	tmpl.custom.repeat = {
		start: function (match) {
			...
			if (args.length === 2) {
				return "for (var " + iteratorVar + " = 0, iterations = parseFloat(" + args[0] + ") + Math.floor(Math.random() * (parseFloat(" + args[1] + ") - parseFloat(" + args[0] + "))); " + iteratorVar + " < iterations; " + iteratorVar + "++) {" + '\n';
			}
	...
	[
		{{repeat 200}}
		{
			...
			"attributes": {
			{{repeat 5, 50}}
				"{{:RandomData('companies')}}": "{{:RandomData('states').name}}",
			{{/repeat}}
			},
